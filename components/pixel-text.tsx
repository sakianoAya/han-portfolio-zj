...
const PixelText: React.FC<PixelTextProps> = ({
  ...
  const pixelGap = Math.max(1, pixelSize / (isClient && window.innerWidth < 640 ? 6 : 4))
  ...
  useEffect(() => {
    ...
    const letterSpacingFactor = getLetterSpacing()
    let totalWidth = 0
    let maxHeight = 0
  ...
  }, [text, size, pixelSize, pixelGap, letterSpacing, isClient])

  // Ensure text is rendered after component mounts
  useEffect(() => {
    if (isInitialized && isClient) {
      renderText()
    }
  }, [isInitialized, currentTheme, isClient])
  ...
  // Re-render when hit characters change or theme changes
  useEffect(() => {
    if (isClient) {
      renderText()
    }
  }, [hitCharacters, currentTheme, isClient])

  // Force re-render on window resize to ensure visibility
  useEffect(() => {
    if (!isClient) return
  ...
  }, [text, hitCharacters, currentTheme, pixelSize, pixelGap, letterSpacing, isClient])
  ...
  useEffect(() => {
    ...
    const handleScreenResize = () => {
      // 重新计算像素大小和间距
      const newPixelSize = getPixelSize()
      const newPixelGap = Math.max(1, newPixelSize / (window.innerWidth < 640 ? 6 : 4))
      const letterSpacingFactor = getLetterSpacing()
      ...
      if (canvasRef.current) {
        ...
        for (let i = 0; i < text.length; i++) {
          ...
          totalWidth += (pixelChar[0]?.length || 0) * (newPixelSize + newPixelGap) + newPixelGap
          maxHeight = Math.max(maxHeight, (pixelChar.length || 0) * (newPixelSize + newPixelGap) + newPixelGap)
        }
